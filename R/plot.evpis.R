...
#'
#' @param lambda_max Optional upper limit for the WTP axis. When \code{NULL},
#'   a heuristic based on the distribution of \eqn{|Delta_Cost / Delta_Effect|}
#'   is used to provide a robust default.
#' @param lambda_steps Number of WTP evaluation points used to compute EVPI.
#' @param return_data Logical; if \code{TRUE}, a tibble containing EVPI values
#'   is returned instead of a plot.
#'
#' @return A \code{ggplot} object or a tibble with EVPI values when
#'   \code{return_data = TRUE}.
#' @rawNamespace export(plot.evpis)
plot.evpis <- function(
    data,
    color_by     = NULL,
    shape_by     = NULL,
