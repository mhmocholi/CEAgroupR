...
This vignette illustrates how to apply the package to *multigroup*
analysis, where multiple datasets (e.g., base case and discounted
scenario) are processed simultaneously, and visualized through a
graphical engine providing a consistent set of aesthetics across all
plots.
...
To run a multigroup analysis, the input must be a **named list** of
datasets:
...
The result is a `cea_results_list` containing analysis results for each
dataset.
...
Descriptive summaries (means, SDs, deltas, ICERs and NMBs) are available
in:
...
All share a unified graphical engine (`ce_plot_base`) ensuring
consistent colour, shape, grouping, faceting and palette behaviour.
